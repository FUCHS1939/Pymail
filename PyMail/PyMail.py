...
def showmaillist():
    ...
    if num % 10:
        ...
        n = num // 10
    for i in range(n):
        getmail_list = server.get_headers(start_index = 1 + 10 * i, end_index = 10 * (i + 1))
        print("ID\t  标题")
        for j in range(len(getmail_list)):
            print("{}\t {}".format(j + 1 + 10 * i, getmail_list[j]))
...
def safe_str(o, max_len=250):
    if o is None:
        return ''
    s = str(o).replace('/', '：').replace('\\','_').replace('*','x').replace('?','_').replace('"','_').replace('<','《').replace('>','》').replace('|','_')
    if len(s) > max_len:
        return s[:max_len]
    return s
